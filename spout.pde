...
void ControlesSpout() {
  ...
  if (nControls > 0) {
    for (int i = 0; i < nControls; i++) {
      ...
      if (controlName[i].equals("Allkeys")) {
        allKeys =  (boolean)(controlValue[i] == 1);
      }
      if (controlName[i].equals("Posrandom")) {
        PosRandom =  (boolean)(controlValue[i] == 1);
      }
      if (controlName[i].equals("Size")) {
        size = controlValue[i];
      }
      if (controlName[i].equals("Amount")) {
        amount = parseInt(controlValue[i]);
      }
      if (controlName[i].equals("TopSpeed")) {
        topSpeed = controlValue[i];
      }
      if (controlName[i].equals("Aceleration")) {
        aceleracion = controlValue[i];
      }
      if (controlName[i].equals("Polygon Edges")) {
        polygonedges = parseInt(controlValue[i]);
      }
      if (controlName[i].equals("Border")) {
        border = controlValue[i];
      }
      ...
      if (controlName[i].equals("Life")) {
        life = controlValue[i];
      }
       if (controlName[i].equals("Rdm")) {
        rdm = controlValue[i];
      }
